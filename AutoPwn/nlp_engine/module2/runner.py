...
OUTPUT_JSON = THIS_DIR.parent / "module3" / "trace_events.json"
...
def run_exploit(tracer_so: Path, solve_py: Path,
    ...
    patched = src
    for name in ("chall_patched", "chall"):
        if (solve_py.parent / name).exists():
            patched = patched.replace(f'_path = "./{name}"', f'_path = "{binary.resolve()}"')
            break
...
def main() -> None:
    ...
    binary = _find_binary(args.solve)
    run_exploit(args.tracer_so, args.solve, args.log, args.drrun, args.timeout)
    events, mmap = parse_log(args.log, binary.name)
